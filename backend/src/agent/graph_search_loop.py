...
def chatbot_explore(state: State):
    ...
    instructions = """
        ...
        When you have no more queries, return the list of products you found.
        ...
        """
    system_prompt = instructions.format(queries=queries)
...
def route_tools(
    ...
    if isinstance(state, list):
        ai_message = state[-1]
    elif messages := state.get("messages_explore", []):
        ai_message = messages[-1]
    elif messages := state.get("messages_research", []):
        ai_message = messages[-1]
    else:
...
def format_products(State: State):
    final_message = State.get("messages_explore", [])[-1]
    ...
    result = llm_with_structured_output.invoke([
        ...
        """.format(query=State.get("query", "")) + final_message.content])
...
graph_builder.add_node("chatbot", chatbot_explore)
...
if __name__ == "__main__":
    ...
    def stream_graph_updates(user_input: str):
        for event in graph.stream({"query": "useful sleep tracker", 
                                   "queries": [        
        ...
        "messages_explore": [{"role": "user", "content": user_input}]}):
            ...
            for value in event.values():
                if "messages_explore" in value:
                    print("Assistant:", value["messages_explore"][-1].content)
                else :
                    print("Products:", json.dumps(value, indent=2))
